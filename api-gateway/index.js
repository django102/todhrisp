...
const { authenticateToken, noAuth } = require("./auth")
...
services.forEach(({ route, target, isAuth }) => {
    // Proxy options
    const proxyOptions = {
        target,
        changeOrigin: true,
    ...
    // Apply rate limiting, timeout and authentication middlewares before proxying
    app.use(
        route,
        rateLimitAndTimeout,
        isAuth ? authenticateToken : noAuth,
